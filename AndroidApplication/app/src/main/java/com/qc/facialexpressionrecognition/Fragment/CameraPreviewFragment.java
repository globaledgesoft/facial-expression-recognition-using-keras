...
public class CameraPreviewFragment extends Fragment {
                    ...
                    for (int i = 0; i < faces.length; i++) {
                        if (faces[i].getScore() > 50) {
                            ...
                            if (rect.left > 0 && rect.top > 0 && rect.left < rect.right && rect.top < rect.bottom) {
                                ...
                                final Bitmap resized = Bitmap.createScaledBitmap(cropped, Constants.BITMAP_WIDTH, Constants.BITMAP_HEIGHT, false);
                                try {
                                    new ImageProcessTask(resized, rect).execute();
                                } catch (Exception e) {
                                    e.printStackTrace();
                                }
                            }
    ...
    private class ImageProcessTask extends AsyncTask<Void, Void, String> {
        ...
        ImageProcessTask(Bitmap inputBitmap, Rect rect) {
        ...
        protected String doInBackground(Void... params) {
            String text = mSnpeHelper.modelInference(inputBitmap);
            return text;
